...
- HDR-intent peak normalization (slider-driven): `src/lib/heif-hdr-processing.ts` (`processHeifHdr`, `decodePrimaryToHdrIntent`, `linearToPq`, `linearToHlg`, `encodeLinearToHdrChannel`), `src/lib/processing-core.ts:1599-1610` (option plumbing), `src/lib/diagnostics-events.ts` — emits `hdr-intent-peak-normalized` and `hdr-intent-peak-normalize-skipped`
- JPEG HDR (Rec.2020 PQ/HLG) classification + decode: `src/lib/jpeg-hdr-processing.ts` (`parseJpegCicpFromApp2`, `isJpegHdrInputCicp`, `processJpegHdr`), routed at `src/lib/processing-core.ts` JPEG branch, classified by `src/lib/processing-path.ts` and `probeInputProcessingPathFromHeaders` — emits `hdr-intent-jpeg-classified`
- Shared HDR-intent and file-family helpers: `src/lib/hdr-intent-dimensions.ts`, `src/lib/image-family.ts`, `src/lib/processing-type-guards.ts`
- HDR-intent dimension cap (wasm32 budget): `HDR_INTENT_MAX_LONG_EDGE` in `src/lib/constants.ts` (== `GMNET_MAX_LONG_EDGE` = 8192). Applied by `processJpegHdr` (via `resizeRasterImageSync`) and `decodePrimaryToHdrIntent` (via `downscale16BitInterleavedRgba`) — emits `hdr-intent-downscaled` (`source: 'heif'|'jpeg'`)
- libultrahdr WASM build: `ultrahdr-wasm/CMakeLists.txt` sets `-fexceptions` on `CMAKE_CXX_FLAGS` and link options so `std::bad_alloc` inside `uhdr_encode` returns `UHDR_CODEC_MEM_ERROR` instead of calling `abort()`
- HEIF HDR decode failure (DaVinci-style): `src/lib/heif-hdr-processing.ts` `decodePrimaryToHdrIntent` emits `hdr-intent-decode-failed` (severity `error`) plus `console.error` with `{ code, bitsPerPixel, primaries, transfer, width, height, filename }` and throws a verbose Error; no fallback path
- HDR intent peak-memory release (MobileSafari): `src/lib/byte-source-release.ts`, `src/lib/hdr-intent-source-release.ts`, `src/lib/processing-core.ts`, `src/lib/heif-hdr-processing.ts`, `src/lib/diagnostics-events.ts` — emits `hdr-intent-source-released`, `hdr-intent-format-downgraded`
...
- Encode-phase peak-memory release (MobileSafari Round 2): `src/lib/byte-source-release.ts`, `src/lib/sdr-pixel-source-release.ts`, `src/lib/gmnet-gain-map-source-release.ts`, `src/lib/compressed-payload-release.ts`, `src/lib/processing-core.ts`, `src/lib/diagnostics-events.ts` — emits `sdr-pixel-source-released`, `gmnet-gain-map-source-released`, `compressed-payload-released`; zeros rotated SDR / GMNet gain-map RGBA and compressed base/gain-map/EXIF JPEG buffers after libultrahdr setters copy into wasm heap
