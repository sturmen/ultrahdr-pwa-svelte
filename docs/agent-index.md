...
- JPEG HDR (Rec.2020 PQ/HLG) classification + decode: `src/lib/jpeg-hdr-processing.ts` (`parseJpegCicpFromApp2`, `isJpegHdrInputCicp`, `processJpegHdr`, `constrainHdrIntentDimensions`), routed at `src/lib/processing-core.ts` JPEG branch, classified by `src/lib/processing-path.ts` and `probeInputProcessingPathFromHeaders` — emits `hdr-intent-jpeg-classified`
