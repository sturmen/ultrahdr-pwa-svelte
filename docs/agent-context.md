...
- `src/lib/hdr-intent-dimensions.ts`, `src/lib/image-family.ts`, `src/lib/processing-type-guards.ts`: shared helpers for HDR-intent long-edge capping, image family detection, and processing-result guards used by processing, preview, and classification code.
...
- `src/lib/byte-source-release.ts`: shared typed byte-source release utility used by processing memory-release wrappers to emit diagnostics, detach large buffers when needed, and zero mutable `.data` views safely.
