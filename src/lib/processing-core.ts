...
export async function processImage(file: File, options: ProcessOptions = {}): Promise<Blob> {
    ...
    try {
        ...
        telemetry.emitStageProgress('generate-gain-map', 0, withProcessingPath({
            substage: 'gmnet-session-pre-module-import',
        }));
        const gmnetSessionModule = await import('./gmnet-session.js');
        telemetry.emitStageProgress('generate-gain-map', 0, withProcessingPath({
            substage: 'gmnet-session-post-module-import',
        }));
        gmnetSessionModule.setGmnetProbeSink((substage, context) => {
            telemetry.emitStageProgress('generate-gain-map', 0, withProcessingPath({
                substage,
                ...(context && typeof context === 'object' ? context : {}),
            }));
        });
        let gainMapGenerationResult;
        try {
            gainMapGenerationResult = await telemetry.runStage(
                'generate-gain-map',
            ...
            gmnetSessionModule.setGmnetProbeSink(null);
        }
        const { gainMapImageData, metadata: generatedGainMapMetadata } = gainMapGenerationResult;
