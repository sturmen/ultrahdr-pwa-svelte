...
let jpegtranWasmModule: {
  ...
  _jpegtran_wasm_transform(state: number, inputPtr: number, inputSize: number, transformCode: number, trim: number, perfect: number): number;
  _jpegtran_wasm_get_output_data(state: number): number;
  _jpegtran_wasm_get_output_size(state: number): number;
  _jpegtran_wasm_get_last_error_code?(state: number): number;
  _jpegtran_wasm_get_last_error_message?(state: number): number;
  _jpegtran_wasm_get_error_image_width?(state: number): number;
  _jpegtran_wasm_get_error_image_height?(state: number): number;
  _jpegtran_wasm_get_error_mcu_width?(state: number): number;
  _jpegtran_wasm_get_error_mcu_height?(state: number): number;
  _jpegtran_wasm_get_error_imperfect_mask?(state: number): number;
...
function getOffendingEdges(mask: number): string[] {
  const edges: string[] = [];
  if ((mask & 1) !== 0) edges.push('right');
  if ((mask & 2) !== 0) edges.push('bottom');
  if ((mask & 4) !== 0) edges.push('left');
  if ((mask & 8) !== 0) edges.push('top');
  return edges;
}
...
function createTransformFailureError(wasm: NonNullable<typeof jpegtranWasmModule>, state: number, transform: string): JpegTransformError {
  ...
  if (rawErrorCode === JPEGTRAN_ERROR_IMPERFECT) {
    ...
    return new JpegTransformError(
      ...
      { transform, width, height, mcuWidth, mcuHeight, offendingEdges: getOffendingEdges(imperfectMask) },
    );
  }
...
export async function rotateJpeg(
  inputBytes: Uint8Array | ArrayBuffer,
  transform: string,
  options: { trim?: boolean; perfect?: boolean } = {},
): Promise<Uint8Array> {
  ...
  if (!wasm) {
    throw new JpegTransformError('Jpegtran WASM module failed to load', 'JPEG_TRANSFORM_INIT_FAILED');
  }
  const state = wasm._jpegtran_wasm_create();
  if (!state) {
    throw new JpegTransformError('Failed to create jpegtran wasm state', 'JPEG_TRANSFORM_INIT_FAILED');
  }

  const inputPtr = wasm._malloc(normalizedBytes.length);
  if (!inputPtr) {
    wasm._jpegtran_wasm_destroy(state);
    throw new JpegTransformError('Failed to allocate memory for JPEG input', 'JPEG_TRANSFORM_ALLOC_FAILED');
  }
