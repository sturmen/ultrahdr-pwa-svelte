...
// node_modules/@monogrid/gainmap-js/dist/compress-BaKKOAXI.js
var canvasToBlob = async (canvas, mimeType, quality) => {
...
// node_modules/@monogrid/gainmap-js/dist/encode.js
var getDataTexture = (image) => {
  let dataTexture;
...
var GainMapEncoderMaterial = class extends ShaderMaterial {
  _minContentBoost;
  _maxContentBoost;
  _offsetSdr;
  _offsetHdr;
   ...
   */
  get gainMapMin() {
    return [Math.log2(this._minContentBoost), Math.log2(this._minContentBoost), Math.log2(this._minContentBoost)];
  }
  /**
   ...
   */
  get gainMapMax() {
    return [Math.log2(this._maxContentBoost), Math.log2(this._maxContentBoost), Math.log2(this._maxContentBoost)];
  }
  /**
...
var SDRMaterial = class extends ShaderMaterial {
  _brightness = 0;
  _contrast = 1;
  _saturation = 1;
  _exposure = 1;
...
var getSDRRendition = (hdrTexture, renderer, toneMapping, renderTargetOptions) => {
...
var encode = (params) => {
  ...
  const gainMapRenderer = getGainMap({
    ...
    image: dataTexture,
    sdr,
    renderer: sdr.renderer
    // reuse the same (maybe disposable?) renderer
  });
  return {
    sdr,
...
var encodeAndCompress = async (params) => {
...
var findTextureMinMax = (image, mode = "max", renderer) => {
  const srcTex = getDataTexture(image);
  if (srcTex.format !== AlphaFormat && srcTex.format !== RGBFormat && srcTex.format !== RGBAFormat && srcTex.format !== DepthFormat && srcTex.format !== DepthStencilFormat && srcTex.format !== RedFormat && srcTex.format !== RedIntegerFormat && srcTex.format !== RGFormat && srcTex.format !== RGIntegerFormat && srcTex.format !== RGBIntegerFormat && srcTex.format !== RGBAIntegerFormat) {
  ...
  const mat = new ShaderMaterial({
    ...
    defines: {
      ...
      INITIAL_VALUE: mode === "max" ? 0 : 65504
...
//# sourceMappingURL=@monogrid_gainmap-js_encode.js.map
